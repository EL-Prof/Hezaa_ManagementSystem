...
namespace POSMainForm
{
    public partial class frmListProduct : Form
    {
        ...
        public void LoadProducts(string strSearch)
        {
            try
            {
                SQLConn.sqL = "SELECT ProductNo, ProductCOde, P.Description, Color, UnitPrice, StocksOnHand, Size, CategoryName FROM Product as P LEFT JOIN Category C ON P.CategoryNo = C.CategoryNo WHERE P.ProductCOde LIKE '%" + strSearch + "%' ORDER BY ProductNo";
                SQLConn.ConnDB();
        ...
        {
            SQLConn.strSearch = Interaction.InputBox("ENTER PRODUCT Code.", "Search Product", " ");
        ...
        private void btnRemove_Click(object sender, EventArgs e)
        {
            if (ListView1.Items.Count == 0)
            {
                Interaction.MsgBox("Please select record to remove", MsgBoxStyle.Exclamation, "RemoveItem");
                return;
            }
            try
            {
